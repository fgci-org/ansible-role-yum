...
ANSIBLE_PLAYBOOk2="tests/test2.yml"
ANSIBLE_PLAYBOOks="$ANSIBLE_PLAYBOOk $ANSIBLE_PLAYBOOk2"
#ANSIBLE_LOG_LEVEL=""
...
function test_playbook_syntax(){
    for book in $ANSIBLE_PLAYBOOks; do
      echo "TEST: ansible-playbook -i ${ANSIBLE_INVENTORY} $book --syntax-check"

      ansible-playbook -i ${ANSIBLE_INVENTORY} $book --syntax-check ||(echo "ansible playbook syntax check was failed" && exit 2 )
    done
}

function test_playbook_check(){
    for book in $ANSIBLE_PLAYBOOks; do
      echo "TEST: ansible-playbook -i ${ANSIBLE_INVENTORY} $book ${ANSIBLE_LOG_LEVEL} --connection=local ${SUDO_OPTION} ${ANSIBLE_EXTRA_VARS} --check"

      ansible-playbook -i ${ANSIBLE_INVENTORY} $book ${ANSIBLE_LOG_LEVEL} --connection=local ${SUDO_OPTION} ${ANSIBLE_EXTRA_VARS} --check ||(echo "playbook check failed" && exit 2 )
    done
...
function test_playbook(){
    for book in $ANSIBLE_PLAYBOOks; do
      echo "TEST: ansible-playbook -i ${ANSIBLE_INVENTORY} $book ${ANSIBLE_LOG_LEVEL} --connection=local ${SUDO_OPTION} ${ANSIBLE_EXTRA_VARS}"
      ansible-playbook -i ${ANSIBLE_INVENTORY} $book ${ANSIBLE_LOG_LEVEL} --connection=local ${SUDO_OPTION} ${ANSIBLE_EXTRA_VARS} ||(echo "first ansible run failed" && exit 2 )
